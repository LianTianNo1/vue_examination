    ...
    <div v-show="containerShow" class="container">
      <div class="left">
        ...
        <leftItem
          ...
          @parentChooseItem="chooseItem($event)"
          :title="'判断题'"
          :data="judge_list_data"
          :answerList="judge_answerList"
          :titletype="'judge'"
        ></leftItem>
        <leftItem
          v-if="leftShow"
          @parentChooseItem="chooseItem($event)"
          :title="'填空题'"
          :data="blank_list_data"
          :answerList="blank_answerList"
          :titletype="'blank'"
        ></leftItem>
        <div
        ...
        <div
          v-show="JSON.stringify(judge_list_data) !== '{}'"
          class="sign_choose"
        >
          <div class="left_title">判断题</div>
          <div class="left_item">
            <div
              @click="chooseItem('judge', index)"
              :key="item[judge_list_data['title_index']]"
              v-for="(item, index) in judge_list_data.data"
              :class="{
                serial: true,
                serial_visited: judge_answerList[index],
              }"
            >
              {{ index + 1 }}
            </div>
